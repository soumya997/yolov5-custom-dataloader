...
class CustomDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        if self.augment:
            # Albumentations
            combined_image, labels_main = self.albumentations(combined_image, labels_main)
            nl = len(labels_main)  # update after albumentations
            ...
            if random.random() < hyp['flipud']:
                combined_image = np.flipud(combined_image)
                if nl:
                    labels_main[:, 2] = 1 - labels_main[:, 2]

            # Flip left-right
            if random.random() < hyp['fliplr']:
                combined_image = np.fliplr(combined_image)
                if nl:
                    labels_main[:, 1] = 1 - labels_main[:, 1]
